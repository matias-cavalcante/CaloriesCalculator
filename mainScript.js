...
function cathegoriesPredictions(names, inputSource) {
  ...
  names.filter(function (cathe) {
    if (
      cathe.name.toLowerCase().startsWith(inputSource.toLowerCase()) == true
    ) {
...
function tagValueReplaceSpace(tag) {
  let newTag;
  if (tag.includes(" ") == true) {
    newTag = tag.replaceAll(" ", "");
    if (newTag.includes("&") == true) {
      return newTag.replaceAll("&", "");
    }
    return newTag;
  } else {
    return tag;
  ...
  fromTag.addEventListener("keydown", function () {
    ...
    options.forEach(function (opt) {
      ...
      div.onclick = function () {
        ...
        const fileName = tagValueReplaceSpace(fromTag.value);
        URL = encodeURI("https://beerapi.herokuapp.com/?" + fileName);
        fetch(URL)
          .then((res) => {
            return res.text();
          })
